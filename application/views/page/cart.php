				...
				<?php foreach ($cart->result_array() as $item) : ?>
					<div class="item">
						<div class="item-main">
							...
							<small class="text-muted qty_product_<?= $item['id']; ?>">Jumlah: <?= $item['qty']; ?></small>
							<!-- <small class="text-muted qty_product_<?= $item['id']; ?>">Jumlah: <?= $item['qty']; ?> <a href="#" class="text-dark" data-toggle="modal" data-target="#qty" onclick="showQty('<?= $item['id']; ?>')"><i class="fa text-info fa-edit"></i></a></small> -->
							<h3 class="price mt-0 mb-0">Rp <?= number_format($item['price'] * $item['qty'], 0, ",", "."); ?></h3>
							<?php if ($item['ket'] == '') { ?>
								<small class="desc_product_<?= $item['id']; ?>"><button class="btn btn-sm btn-primary" href="#" class="text-dark" data-toggle="modal" data-target="#modalAddDescription" onclick="showModalAddKet('<?= $item['id']; ?>')">Tambah catatan</button></small>
							<?php } else { ?>
								<small class="desc_product_<?= $item['id']; ?>">Catatan: <?= $item['ket']; ?> <a href="#" class="text-dark" data-toggle="modal" data-target="#modalEditDescription" onclick="showModalEditKet('<?= $item['id']; ?>')"><i class="fa text-info fa-edit"></i></a></small>
							<?php } ?>
...
<!-- Modal -->
<div class="modal fade" id="qty" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalCenterTitle">Edit jumlah</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body" id="bodyQty">

			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" id="btnQty" data-dismiss="modal">Simpan</button>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	function showQty(id) {
		$.ajax({
			...
			success: function(res) {
				$("#bodyQty").html(`<div class="form-group">
                    <input type="number" name="ket_product" id="ket_qty_edit" class="form-control form-control-sm" value=${res.qty}>
                    <input type="hidden" id="rowid_pro_edit" value=${id}>
                </div>`);
			}
		})
	...
	$("#btnSaveKetProduct").on('click', function() {
		...
		$.ajax({
			...
			success: function() {
				// $("small.desc_product_" + rowid).html("Catatan: " + ket + ` <a href="#" class="text-dark" data-toggle="modal" data-target="#modalEditDescription" onclick="showModalEditKet('${rowid}')"><i class="fa text-info fa-edit"></i></a>`);
	...
	$("#btnQty").on('click', function() {
		...
		$.ajax({
			...
			success: function() {
				// $("small.qty_product_" + rowid).html("Jumlah: " + qty + ` <a href="#" class="text-dark" data-toggle="modal" data-target="#modalEditDescription" onclick="showModalEditKet('${rowid}')"><i class="fa text-info fa-edit"></i></a>`);
				location.reload(true);
			}
		})
	})
