	...
	<div class="row">
		<div class="col-md-7">
			...
			<?php if ($metod == 'cod') { ?>
				...
				<a href="https://wa.me/<?= $this->config->item('whatsappv2'); ?>?text=Halo kak, saya membeli produk di <?= $this->config->item('app_name'); ?> menggunakan metode COD/Cash of Delivery dengan Order ID <?= $invoice_id['invoice']; ?>" class="btn btn-sm btn-success" target="_blank"> Hubungi Whatsapp</a>
			<?php } else { ?>
				...
				<?php endforeach; ?>
				<p>Jika sudah melakukan pembayaran, silakan melakukan konfirmasi pembayaran <a href="<?= base_url(); ?>payment/confirmation" class="btn btn-info btn-sm">dengan klik disini</a> atau bisa melalui Whatsapp <?= $this->config->item('whatsapp') ?> atau <a href="https://wa.me/<?= $this->config->item('whatsappv2') ?>" target="_blank">klik disini</a> dengan format sebagai berikut:</p>
				<table>
					<tr>
