		...
		if ($this->form_validation->run() == false) {
			...
			if ($check) {
				...
				if ($upload['result'] == 'success') {
					...
					$this->email->subject('Konfirmasi Pembayaran ' . $invoice);
					$this->email->message(
						'<p>Halo admin, <br/>Pesanan dengan ORDER ID ' . $invoice . ' telah dilakukan pembayaran. Periksa buktinya dengan <a href="' . base_url() . 'administrator/proof/" target="_blank">klik disini</a></p>
            '
					);
					$this->email->send();
